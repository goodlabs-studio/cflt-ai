...
2. **`schemaRegistrySubject`** — reference a schema that's *already* registered, by exact subject name (`{"value": "<exact-subject>"}`). Use this for any existing Confluent Cloud topic.
   ...
   ⚠️ **This setting only controls which subject ShadowTraffic downloads the schema definition from for its own generation/validation — it does not retarget which subject the underlying `KafkaAvroSerializer` looks up or registers against at produce time.** That's governed purely by the standard Confluent client config `value.subject.name.strategy` (default `TopicNameStrategy`, `<topic>-value` — see [Subject Name Strategy](https://docs.confluent.io/cloud/current/sr/fundamentals/serdes-develop/index.md#sr-schemas-subject-name-strategy)), which nothing in `schemaRegistrySubject` changes. If your registered subject doesn't follow `TopicNameStrategy` and you don't *also* set `producerConfigs.value.subject.name.strategy` to match, produce calls will look up the `TopicNameStrategy`-derived subject regardless of what `schemaRegistrySubject` says — confirmed live: pointing `schemaRegistrySubject` at a real, non-default subject name did not stop `KafkaAvroSerializer` from requesting the `TopicNameStrategy`-derived one and failing with `Subject '<topic>-value' not found`.
   - **Simplest fix, and the recommended default:** register your schema under the plain `TopicNameStrategy` subject (`<topic>-value`) and drop `schemaRegistrySubject` entirely — nothing needs overriding.
   - If you must use a non-default subject, set **both** `schemaRegistrySubject` (for ShadowTraffic) **and** `producerConfigs.value.subject.name.strategy` (for the serializer) to strategies/values that resolve to the same subject — otherwise the two will silently diverge.
   - **A second, sharper gotcha:** if the subject `schemaRegistrySubject` names doesn't exist, ShadowTraffic does not fail with a clear 404 — it silently falls back to a schema it infers itself from the generator's `value` field map (auto-named from the topic, e.g. topic `foo.bar` → record `barValue` in namespace `foo`). That inferred schema then fails Confluent's compatibility check against whatever's actually registered, surfacing as a confusing `NAME_MISMATCH` `SerializationException` that names a schema you never wrote, not an obvious "subject not found" error.

To point the same config at a **new** schema/topic: (1) confirm the target subject name *and* which `value.subject.name.strategy` your producer config uses (default `TopicNameStrategy`, so usually just confirm the subject is `<topic>-value`), (2) rewrite the generator's `value` field map to match the new Avro record's fields, (3) register the schema under that subject — via `avroSchemaHint` if it doesn't exist yet, or confirm it's already there if using `schemaRegistrySubject`. The `connections` block, credential file, and run command don't change.
...
- `schemaRegistrySubject` does **not** make ShadowTraffic produce against a non-default subject — it only tells ShadowTraffic which subject to *read* the schema from. The subject actually used at produce time is whatever `value.subject.name.strategy` computes (default `TopicNameStrategy`, `<topic>-value`), independent of this setting. Confirm the registered subject matches `<topic>-value` (the common case) rather than trying to redirect produce-time lookups via `schemaRegistrySubject` alone. See `patterns/topic-naming.md` for the naming-strategy background.
- If `schemaRegistrySubject` names a subject that doesn't exist, expect a misleading `NAME_MISMATCH` `SerializationException` (ShadowTraffic silently substitutes a self-inferred schema) rather than a clear "subject not found" error — don't assume the compatibility error's schema is one you wrote.
