...
```mermaid
flowchart TD
  subgraph NS["Kubernetes namespace: confluent"]
    KAFKA["Kafka StatefulSet — broker pods"]
    COMP["SR / Connect / ksqlDB pods"]
    KRAFT["KRaft controller pods"]
    KJMX["jmx exporter sidecar"]
    CJMX["jmx exporter sidecar"]
    SM["ServiceMonitor (Prom Operator)"]
    KRAFT
    KAFKA --> KJMX
    COMP --> CJMX
    KJMX -->|":7203/metrics"| SM
    CJMX -->|":7203/metrics"| SM
  end
  PROM["Prometheus"]
  GRAF["Grafana"]
  SM --> PROM
